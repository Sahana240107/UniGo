...
/**
 * src/app/(auth)/rider/email/page.tsx
 * RiderEmailEntryScreen — sends verification email and navigates to polling screen.
 */

import { useState } from 'react';
import { useRouter } from 'next/navigation';
import Link from 'next/link';
import { sendRiderEmailVerification } from '@/lib/authService';

const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

export default function RiderEmailPage() {
  ...
  const handleSend = async () => {
    ...
    setError('');
    try {
      await sendRiderEmailVerification(email.trim().toLowerCase());
      router.push(`/rider/verify-email?email=${encodeURIComponent(email.trim().toLowerCase())}`);
    } catch (err: any) {
  ...
  return (
    <main className="min-h-screen bg-[#F8F7FF] flex flex-col px-6 py-8">
      ...
      <div className="flex flex-col items-center mb-10">
        ...
        <h1 className="text-2xl font-extrabold text-[#1A1A2E] mb-2">Continue as Rider</h1>
