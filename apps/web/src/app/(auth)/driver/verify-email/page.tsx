...
/**
 * src/app/(auth)/driver/verify-email/page.tsx
 * DriverEmailVerifyWaiting — polls for email verification.
 *
 * On success:
 *   existing driver (active)          → /driver/dashboard
 *   existing driver (pending/action)  → /driver/pending
 *   new driver                        → /driver/setup (profile + docs)
 */

import { useState, useEffect, useRef, useCallback, Suspense } from 'react';
import { useRouter, useSearchParams } from 'next/navigation';
import { checkDriverEmailVerified, resendDriverEmailVerification } from '@/lib/authService';
...
function DriverVerifyContent() {
  ...
  const handleVerified = useCallback(
    (result: {
      ...
      if (!result.is_new_user && result.user) {
        ...
        // Route existing driver based on their submission state
        const state = result.driver_profile?.submission_state;
        if (state === 'active') {
        ...
        // New driver → setup form
        router.push(
          `/driver/setup?idToken=${encodeURIComponent(result.idToken)}&uid=${encodeURIComponent(result.firebase_uid)}&email=${encodeURIComponent(email)}`,
        );
      }
    },
  ...
  const poll = useCallback(async () => {
    ...
    try {
      const result = await checkDriverEmailVerified(email);
  ...
  const handleResend = async () => {
    setResending(true);
    try {
      await resendDriverEmailVerification(email);
      setCooldown(30);
    } catch (err: any) {
    ...
    <main className="min-h-screen bg-[#F8F7FF] flex flex-col px-6 py-8">
      ...
      <div className="flex flex-col gap-4 mb-8">
        {[
          ...
          { n: 3, text: "Come back here — we'll continue your setup" },
        ].map(({ n, text }) => (
